...
#jwt get grad
gv = optim.compute_gradients(output[-1])
# compute gradients
...
#grad_vals = session.run([g for (g,v) in gv], feed_dict=get_feed_dict(xinput, ytarget))
grad_vals = [g for (g,v) in gv]
#print("grad_vals=", session.run([g for (g,v) in gv], feed_dict=get_feed_dict(xinput, ytarget)))#grad_vals)
dwix = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [0, 0], [input_size, hidden_size]), perm=[1, 0]), [input_size*hidden_size])
dwcx = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [0, hidden_size], [input_size, hidden_size]), perm=[1, 0]), [input_size*hidden_size])
dwfx = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [0, hidden_size*2], [input_size, hidden_size]),perm=[1, 0]), [input_size*hidden_size])
dwox = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [0, hidden_size*3], [input_size, hidden_size]),perm=[1, 0]), [input_size*hidden_size])
dwih = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [input_size, 0], [hidden_size, hidden_size]),perm=[1, 0]), [hidden_size*hidden_size])
dwch = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [input_size, hidden_size], [hidden_size, hidden_size]),perm=[1, 0]), [hidden_size*hidden_size])
dwfh = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [input_size, hidden_size*2], [hidden_size, hidden_size]),perm=[1, 0]), [hidden_size*hidden_size])
dwoh = tf.reshape(tf.transpose(tf.slice(grad_vals[0], [input_size, hidden_size*3], [hidden_size, hidden_size]),perm=[1, 0]), [hidden_size*hidden_size])
dbi = tf.slice(grad_vals[1], [0], [hidden_size])
dbc = tf.slice(grad_vals[1], [hidden_size], [hidden_size])
dbf = tf.slice(grad_vals[1], [hidden_size*2], [hidden_size])
dbo = tf.slice(grad_vals[1], [hidden_size*3], [hidden_size])
ref_result = session.run(tf.concat([dwix, dwcx, dwfx, dwox, dwih, dwch, dwfh, dwoh, dbi, dbc, dbf, dbo], 0), feed_dict=get_feed_dict(xinput, ytarget))
#print("dwix=", session.run(dwix, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dwcx=", session.run(dwcx, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dwfx=", session.run(dwfx, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dwox=", session.run(dwox, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dwih=", session.run(dwih, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dwch=", session.run(dwch, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dwfh=", session.run(dwfh, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dwoh=", session.run(dwoh, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dbi =", session.run(dbi, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dbc =", session.run(dbc, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dbf =", session.run(dbf, feed_dict=get_feed_dict(xinput, ytarget)))
#print("dbo =", session.run(dbo, feed_dict=get_feed_dict(xinput, ytarget)))

#intelLSTM training
lstm_module = tf.load_op_library('/home/wentingj/tf_bind_rnn/train/lstm_trainingOP.so')
...
#dwfx, dwix, dwcx, dwox, dwfh, dwih, dwch, dwoh, dbf, dbi, dbc, dbo, dx
dall = lstm_module.intel_lstm_train(xinput_intel, (w_x_intel), (w_h_intel), (b_intel), (h_0), (c_0))
data_size = input_size*hidden_size*4 + hidden_size*hidden_size*4 + hidden_size*4
#intel_result_ = dall[0 : data_size] 
intel_result = session.run(dall)
#print(intel_result)
print("--dwx:")
for i in range(input_size*hidden_size*4):
    print(intel_result[i])
    print(ref_result[i])
print("--dwh:")
for i in range(hidden_size*hidden_size*4):
    print(intel_result[i+input_size*hidden_size*4])
    print(ref_result[i+input_size*hidden_size*4])
print("--db:")
for i in range(hidden_size*4):
    print(intel_result[i+input_size*hidden_size*4+hidden_size*hidden_size*4])
    print(ref_result[i+input_size*hidden_size*4+hidden_size*hidden_size*4])
#print("diff=", session.run((dall[0 : input_size*hidden_size*4 + hidden_size*hidden_size*4 + hidden_size*4] - ref_result)/ref_result*100, feed_dict=get_feed_dict(xinput, ytarget)))
...
#session.run(output_intel)
#start = time.time()
#for i in range(0, loops):
#    session.run(lstm_module.intel_lstm(xinput_intel, (w_x_intel), (w_h_intel), (b_intel), (h_0), (c_0)))
#end = time.time()
#print("Intel Forward:")
#print("--- %i samples in %s seconds (%f samples/s, %.7f s/sample) ---" % (forward_samples, (end - start), forward_samples / (end - start), (end - start) / forward_samples))
